...
@Command(name = "getEvents", usage = "getEvents [--name [eventName]] [--start-date [(dd/MM/yyyy)]] [--end-date [(dd/MM/yyyy)]] [--start-time [(HH:mm)]] [--end-time [(HH:mm)]]", description = "get user events that match the given filters")
public class GetEvents extends CommandBase
{
    ...
    public void execute(SessionManager sessionManager, String[] args)
    {
        ...
        Request request;
        if (filters.isEmpty())
            request = new Request(RequestType.GET_EVENTS);
        else
            request = new Request(RequestType.GET_EVENTS, filters);
